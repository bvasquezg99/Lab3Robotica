...
# Conclusiones
1. A la hora de realizar una aplicación de cinemática inversa, se trabaja con las poses deseadas en el efector final del robot. Sin embargo, se debe tener en cuenta que no se tiene una única solución para la pose deseada. Por lo cual es importante tener cuidado de no estar alternando entre soluciones a lo largo de una trayectoria  ya que en el caso del PhantomX se tienen soluciones de codo arriba y codo abajo
2. También en cuanto a una trayectoria, se debe tener en cuenta que así se defina la pose de inicio y la pose final, es importante especificarle al robot, como debe llegar ahí.  En específico, si por ejemplo se usa ctraj, como se hizo en este laboratorio, se debe tener en cuenta que el robot debe tener varios puntos a lo largo de la trayectoria, para poder así realizar de manera adecuada el movimiento, y así evitar problemas como colisiones o sobreexigir a los motores.
3. Durante el desarrollo de ambos puntos del laboratorio, se notó un problema a la hora de realizar un movimiento, y es el de las vibraciones del robot. Si se está haciendo una aplicación algo precisa, como puede ser el caso de un pick and place, las vibraciones pueden alterar el resultado final, para eso lo ideal sería poder fijar el robot a la superficie.
4. Otro factor a tener en cuenta, es que estamos trabajando de forma serial. Es decir, que no se pueden mover todos los motores a la vez para llegar a una pose determinada, sino que el robot mueve una articulación a la vez. Es por eso, que hay que tener cuidado, ya que cada movimiento de articulación puede causar algún tipo de colisión, antes de llegar a la posición objetivo.
5. Durante el desarrollo del laboratorio, se hizo un cambio de Python a MATLAB, este se dió, pese a la facilidad de visualización y simulación que ofrece MATLAB , y así poder verificar los scripts antes de realizar el movimiento con el Phantom.
