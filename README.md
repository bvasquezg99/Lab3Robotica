...
# Conclusiones
1. A la hora de realizar una aplicación de cinemática inversa, se trabaja con las poses deseadas en el efector final del robot. Sin embargo, se debe tener en cuenta que no se tiene una sola solución para la pose deseada. Es importante tener en cuenta, que en el caso del PhantomX se tienen soluciones codo arriba y codo abajo, y se debe tener cuidado de no estar alternando entre estas a lo largo de una trayectoria.
2. También en cuanto a una trayectoria, se debe tener en cuenta que así se definan la pose de inicio y la pose final, es importante especificarle al robot, como debe llegar ahí.  En específico, si por ejemplo se usa ctraj, como se hizo en este laboratorio, se debe tener en cuenta que el robot debe tener varios puntos a lo largo de la trayectoria, para poder así realizar de manera corrwectamente l movimiento, y así evitar problemas como colisiones o sobreexigir a los motores.
...
5. Durante el desarrollo del laboratorio, se hizo un cambio de Python a MATLAB, este se dió, debido a la facilidad de visualización y simulación que ofrece MATLAB , y así poder verificar lso scripts antes de realizar el movimiento con el Phantom.
